...
const MapComponent = ({
  ...
  // TODO: Temporary fetch reflections
  // useEffect(() => {
  //   if (!user || !user.id) return;
  //   const fetchReflections = async () => {
  //     const reflections = await getReflectionsForUser(user.id);
      ...
      >
        ...
        {mapReflections.map((reflection) => (
          <Marker
            key={reflection.id}
            coordinate={{
              latitude: reflection.location?.lat || 0,
              longitude: reflection.location?.long || 0,
            }}
